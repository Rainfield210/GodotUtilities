...
public static class CompilationExtensions {
        ...
        void ResolveDuplicates() {
            if (symbols.Skip(1).Any()) {
                ...
                if (symbols.Skip(1).Any()) {
                    ...
                    symbols = symbols.Where(x =>
                        x.Locations.Select(x => x.GetLineSpan().Path).Any(x => x.EndsWith(hint)));
    ...
    public static string ValidateTypeIgnoreCase(this Compilation compilation, string assemblyName,
        string namespaceName, string type) {
